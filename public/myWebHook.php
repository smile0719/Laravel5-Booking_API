...
//$ret = shell_exec("git pull origin development");
//die($ret);
// Your remote name
$remote = "origin";
...
$aliases = array(
  "development"  => "/home/wodebox/source/fcc-api"
...
if ( isset( $payload['push'] ) ) {
  ...
  $branch = isset( $lastChange['name'] ) && ! empty( $lastChange['name'] ) ? $lastChange['name'] : 'NO-BRANCH-DETECTED-DO-NOT-CREATE-A-FOLDER-WITH-THIS-NAME';
} else {
...
foreach($dirs_to_update as $dir) {
  ...
  $verbose = shell_exec("git pull $remote $branch");
